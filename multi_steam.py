...
class MultiBrowser(QMainWindow):
    ...
    def _showEmptyState(self) -> None:
        for i in range(self.browserLayout.count() - 1, -1, -1):
            ...
            if w:
                w.setParent(None)
        placeholder = QLabel("Нет профилей. Нажмите ‘➕ Профиль’ слева.")
    ...
    def createProfile(self, name: str, doSwitch: bool) -> None:
        """
        ЖЁСТКАЯ изоляция для каждого профиля:
        - создаём НОВЫЙ QWebEngineProfile(self) БЕЗ имени,
        - задаём УНИКАЛЬНЫЙ persistentStoragePath,
        - собственный дисковый кэш и политика постоянных куки.
        """
        pdir = self.profileDir(name)
        profile = QWebEngineProfile(self)  # без имени: Qt не переиспользует раздел
        ...
        def on_download(req: QWebEngineDownloadRequest) -> None:
            ...
            def progress_update():
                ...
                if total > 0:
                    self.statusBar().showMessage(
                        f"Загрузка {file_name}: {received}/{total}", 150
                    )
            ...
            def on_state(st: QWebEngineDownloadRequest.DownloadState):
                if st == QWebEngineDownloadRequest.DownloadState.DownloadCompleted:
                    self.statusBar().showMessage(
                        f"Загрузка завершена: {dirp}/{file_name}", 4000
                    )
                elif st in (
                    ...
                    self.statusBar().showMessage(
                        f"Загрузка прервана: {file_name}", 4000
                    )
...
class BrowserView(QWebEngineView):
    ...
    def createWindow(self, _type: QWebEnginePage.WebWindowType) -> QWebEngineView:
        # поп-апы используют ТОТ ЖЕ profile, что и текущий аккаунт
